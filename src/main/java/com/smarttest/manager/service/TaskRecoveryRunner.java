...
public class TaskRecoveryRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        log.info("Recovering {} running manager tasks from database", runningTaskIds.size());
        for (String managerTaskId : runningTaskIds) {
            try {
                ...
                log.info("Resumed polling for managerTaskId={}", managerTaskId);
            } catch (Exception e) {
                log.error("Failed to recover managerTaskId={}", managerTaskId, e);
            }
        }
    }
}
